...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  static async register(metadata, options = {}, jwsOptions = {}) {
    const {initialAccessToken, jwks, ...clientOptions} = options;
    const {signingKey, keyId, algorithm: signingAlgorithm} = jwsOptions;
    ...
    if (signingKey) {
      ...
      metadataJwt = await new jose.CompactSign(new TextEncoder().encode(metadataJsonStr))
        .setProtectedHeader({
    ...
    const response = await request.call(this, {
      headers: {
        ...
        'Content-Type': metadataJwt ? 'application/jwt' : 'application/json',
        ...(initialAccessToken
          ? {
      ...
      responseType: 'json',
      json: !metadataJwt ? metadata : undefined,
      body: metadataJwt ? metadataJwt : undefined,
  ...
  static async deleteRegistration(clientID, options = {}, jwsOptions = {}) {
    const { initialAccessToken, jwks, ...clientOptions } = options;
    assertIssuerConfiguration(this.issuer, 'registration_endpoint');
    ...
    const response = await request.call(this, {
      ...
      responseType: 'json',
      url: `${this.issuer.registration_endpoint}/${clientID}`,
      method: 'DELETE',
    });
    const responseBody = processResponse(response, { statusCode: 204, bearer: true, body: false });
